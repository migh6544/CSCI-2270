...
/* 4.1.2 */
    ...
    {
        //use bst.cpp function to insert mail into bst at index j 
        BST.insertMail(testdata[j]);
    }
    ...
    duration<float> duration_insert = duration_cast<nanoseconds>(stop_insert - start_insert);
...
/* 4.1.3 */
    ...
    //calculate duration
     duration<float> duration_search = duration_cast<nanoseconds>(stop_search - start_search);
...
//write to file
ofstream output_BST("output_BST_B.csv");

//column labels 
output_BST << "Insert" << " , " << "Search" << endl;

//write data to output file
for (int n=0; n<400; n++)
{
    output_BST << insert[n] << " , " << search[n] << endl;
